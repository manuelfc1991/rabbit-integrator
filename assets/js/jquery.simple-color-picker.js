jQuery.fn.simpleColorPicker = function (options) {
  var defaults = {
    colorsPerLine: 8,
    colors: [
      "#000000",
  ...
  return this.each(function () {
    ...
    box.find("li.color-box").click(function () {
      ...
      if (jQuery.isFunction(defaults.onChangeColor)) {
        defaults.onChangeColor.call(
          txt,
          opts.colors[this.id.substr(this.id.indexOf("-") + 1)]
        );
      }
      hideBox(box);
    });
    ...
    function hideBox(box) {
      if (opts.hideEffect == "fade") box.fadeOut();
      else if (opts.hideEffect == "slide") box.slideUp();
      else box.hide();
    }

    function showBox(box) {
      if (opts.showEffect == "fade") box.fadeIn();
